...
from .forms import FreePostForm, FreeAdminPostForm, FreeCommentForm
...
def index(request):
    ...
    notices = FreePost.objects.filter(notice=True)
...
def detail(request, post_id):
    ...
    firstcomments = FreeComment.objects.filter(depth=0, post=post.id)
    secondcomments = FreeComment.objects.filter(depth=1, post=post.id)
...
def edit(request, post_id):
    ...
    if request.method == 'POST':
        if current_user.is_superuser:
            form = FreeAdminPostForm(request.POST, instance=post)
        else:
        ...
        if current_user.is_superuser:
            form = FreeAdminPostForm(instance=post)
        else:
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if request.user != user:
                notify.send(request.user, recipient=user, actor=request.user, description='free',
                    verb='add comment your post : '+post.title, target=post, nf_type='add_comment')

            return redirect('free:detail', post.id)
